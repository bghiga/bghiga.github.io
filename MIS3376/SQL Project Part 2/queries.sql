...
--Phase 2
--1
select fl_flight_no, flight.fl_orig, flight.fl_dest
from flight inner join airport
on fl_orig=air_code
where air_hub_airline is null or airport.air_hub_airline = 'American';

--2
select departures.dep_flight_no, departures.dep_dep_date, equip_type.eq_equip_type
from departures 
inner join equip_type
on departures.dep_equip_no=equip_type.eq_equip_no
where equip_type.eq_equip_type like '%CON%' order by dep_dep_date, dep_flight_no;

--3
select pil_pilot_id, pil_pilotname
from pilots
where pil_pilot_id not in(select pil_pilot_id
from pilots
join departures
on pilots.pil_pilot_id=departures.dep_pilot_id);

--4
select pil_pilot_id, pil_pilotname
from pilots
left outer join departures
on pilots.pil_pilot_id=departures.dep_pilot_id
where departures.dep_dep_date is null;

--5
select passenger.pas_name, ticket.tic_seat as "Seat Number"
from passenger, ticket
where passenger.pas_itinerary_no=ticket.tic_itinerary_no
and ticket.tic_flight_no=101 and ticket.tic_flight_date= DATE '2017-07-15'
order by "Seat Number";

--6
select departures.dep_flight_no as "Flight Number", departures.dep_dep_date as "Date", count(*) as "Number Of Passengers"
from departures
join ticket on departures.dep_flight_no=ticket.tic_flight_no
group by departures.dep_flight_no, departures.dep_dep_date
having count(*)>5
order by departures.dep_flight_no, departures.dep_dep_date;

--7
select fl_flight_no, fl_orig, fl_dest
from flight join reservation on fl_flight_no=res_flight_no
where res_name= 'Andy Anderson'
order by fl_flight_no;

--8
select a.air_location as "Departs From", fl_dest as "Arrives at", min(fl_fare) as "Minimum Fare"
from flight join airport a on fl_orig=air_code
group by a.air_location, fl_dest;
